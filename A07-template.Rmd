...
Trip <- Trips %>%
  ...
  group_by(client) %>%
  summarize(duration = as.numeric(edate-sdate)) %>%
  filter(duration < 300)



Trip %>%
  ggplot(aes(x = client, y = duration, color = client)) +
  geom_boxplot() 


```
The graph shows that registered clients have a much higher average duration time (in minutes) than those who are considered casual clients.
...
Trip2 %>%
  ggplot(aes(x = lubridate::yday(sdate)), y = duration) +
  geom_density()

```

The graph shows the rentals that take place in the later months of the year. More people use rentals during the fall rather than the winter.
...
Trip2 %>%
  ggplot(aes(x = lubridate::wday(sdate)), y = duration) +
  geom_density()
```


The graph shows a fairly equal amount of rental duration throughout the week. The durations go up slightly towards the end of the week but drop on the weekends.
...
Trip2 %>%
  ggplot(aes(x = lubridate::hour(sdate)), y = duration) +
  geom_density()
```


Barely any rentals take place in the early hours of the day, but two peaks are seen at around 8 in the morning, and 5-6 in the evening.
...
Trip2 %>%
  ggplot(aes(x = lubridate::minute(sdate)), y = duration) +
  geom_density()
```

There is not much of a pattern to be seen when it comes to the duration depending on the minute of the hour it is being taken out.
...
### My Code here
Trips3 <- Trip2 %>%
  group_by(lubridate::hour(sdate), lubridate::wday(sdate), client) %>%
  summarise(count = n())

Trips3 %>%
  ggplot(aes(x = count, y = lubridate::hour(sdate)), group = lubridate::wday(sdate)) +
  geom_line() +
  facet_wrap( ~ client)
```
